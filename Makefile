...
LDFLAGS += -X $(PREFIX).portManagerTag=3.1.2
LDFLAGS += -X $(PREFIX).kubeletTag=3.0.6
LDFLAGS += -X $(PREFIX).operatorTag=3.4.6
LDFLAGS += -X $(PREFIX).proxyTag=3.0.2
LDFLAGS += -X $(PREFIX).routerTag=3.2.0
LDFLAGS += -X $(PREFIX).controllerTag=3.4.1
...
GO_SDK_MODULE = iofog-go-sdk/v3@v3.4.12
OPERATOR_MODULE = iofog-operator/v3@v3.4.6
REPORTS_DIR ?= reports
