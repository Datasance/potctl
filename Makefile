...
LDFLAGS += -X $(PREFIX).portManagerTag=3.0.1
LDFLAGS += -X $(PREFIX).kubeletTag=3.0.1
LDFLAGS += -X $(PREFIX).operatorTag=3.2.3
LDFLAGS += -X $(PREFIX).proxyTag=3.0.1
LDFLAGS += -X $(PREFIX).routerTag=3.0.1
LDFLAGS += -X $(PREFIX).controllerTag=3.0.5
LDFLAGS += -X $(PREFIX).agentTag=3.0.2
LDFLAGS += -X $(PREFIX).controllerVersion=3.0.5
LDFLAGS += -X $(PREFIX).agentVersion=3.0.2
LDFLAGS += -X $(PREFIX).repo=ghcr.io/datasance
GO_SDK_MODULE = iofog-go-sdk/v3@v3.3.5
